...
function getResources(application) {
  ...
  switch (application.state) {
      ...
      res.secondLine = (
        <h3 className="text-sm">
          ...
          <span className="font-medium">
            {application.step === 1 ? "Adviser" : "Clearance Officer"}
          </span>{" "}
          {relativeTime.from(application.dateReturned)}
        </h3>
      );
      res.thirdLine = (
        <p className="font-light text-xs mt-2">{application.returnRemarks}</p>
      );
      break;
    case "APPROVED":
      ...
      res.secondLine = (
        <h3 className="text-sm">
          Cleared {relativeTime.from(application.dateApproved)}
        </h3>
      );
      res.thirdLine = "";
      break;
    case "PENDING":
      ...
      res.secondLine = (
        <h3 className="text-sm">
          Submitted {relativeTime.from(application.dateSubmitted)}
        </h3>
      );
      res.thirdLine = (
